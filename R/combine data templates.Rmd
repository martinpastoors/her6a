...
vesselnames <- data.frame(
    vessel     = c("KW172"    , "PH2200" , "SCH6",  "FR244"      , "PD365"      , "N905"),
    vesselname = c("Dirk Dirk", "Wiron 6", "Alida", "Christina S", "Kings Cross", "Voyager"), 
...
haul %>%
  ...
  group_by(vessel, surveyarea) %>% 
  summarize(catch=sum(catch,na.rm=TRUE)) %>% 
  dcast(vessel ~ surveyarea, sum, value.var="catch", margins=TRUE) %>% 
  pandoc.table(.,
             style = "simple",
             split.tables=400, 
             justify = "left",
...
haul %>%
  ...
  mutate(vesseltrip = paste(vessel,trip,sep="_")) %>%
