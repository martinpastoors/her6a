...
bio <- 
  ...
  mutate(vessel = gsub("\\s+", "", str_trim(toupper(vessel))),
         ...
         mat    = ifelse(!is.na(maturitystage.1.4.), maturitystage.1.4., mat),
...
t3 <-
  rbind(t1,t2) %>%
  gather(key=variable, value=number, all:genetics) %>% 
...
haul %>%
  ...
  labs(x = NULL, y = NULL, title="2017 haul positions of survey (S) and commercial (C) hauls") +
...
t1 %>%
  ...
  geom_polygon(data=areas, aes(long, lat, group=area), colour="red", size=0.2,fill=NA) +
  # geom_jitter(aes(size=catch, colour=factor(typehaul)), alpha = 0.6) +
  geom_jitter(aes(colour=factor(variable)), alpha = 0.6, size=2) +
...
length %>% 
  ...
  filter(species == "HER") %>% 
  group_by(triphaul, surveyarea, length) %>% 
  summarize(count = sum(count, na.rm=TRUE)) %>% 
  group_by(triphaul, surveyarea) %>% 
  ...
  ggplot(aes(length, triphaul, group=triphaul, height=prop)) +
  theme_publication() +
  theme(strip.background = element_rect(colour=NA, fill = "#f0f0f0"),
        panel.border     = element_rect(colour="gray" , size=0.2),
        legend.title=element_blank() ) +
  geom_joy(stat="identity", scale=2, alpha=0.6) +
...
# plot of length by haul with ggridges for Wiron comparing between fresh survey samples and commercial samples
t <-
  bio %>% 
  filter(vessel == "PH2200") %>% 
  mutate(triphaul = paste(trip,str_pad(haul, 2, pad = "0"),sep="")) %>% 
  filter(species == "HER") %>% 
  group_by(vessel, haul, length, tmp) %>% 
  summarize(count = n()) %>% 
  group_by(vessel, haul, tmp) %>% 
  mutate(prop = count / sum(count, na.rm=TRUE)) %>% 
  filter(!is.na(haul)) 

t %>%   
  ggplot(aes(length, haul, group=haul, height=prop)) +
  theme_publication() +
  theme(strip.background = element_rect(colour=NA, fill = "#f0f0f0"),
        panel.border     = element_rect(colour="gray" , size=0.2),
        legend.title=element_blank() ) +
  # geom_ridgeline(aes(fill=factor(tmp)), stat="identity", scale=2, alpha=0.6) +
  geom_ridgeline(data=filter(t, vessel=="PH2200" & tmp =="C"), scale = 5, fill="blue", alpha=0.6) +
  geom_ridgeline(data=filter(t, vessel=="PH2200" & tmp =="S"), scale = 5, fill="red",  alpha=0.6) +
  scale_y_continuous(trans="reverse", breaks=unique(t$haul))

  # facet_wrap(~tmp, ncol=2)
...
mat <- 
  bio %>% 
  filter(!is.na(mat)) %>% 
  group_by(species, surveyarea, date, length, mat9points) %>% 
  summarize(n =n()) %>% 
  group_by(species, surveyarea, date, length) %>% 
  mutate(prop = n / sum(n, na.rm=TRUE))
...
length %>%
  ...
  # mutate(date     = as.character(date)) %>% 
  filter(species == "HER") %>% 
  filter(!is.na(mat9points)) %>% 
  bind_rows(data.frame(surveyarea="1")) %>% 
  group_by(date, mat9points, surveyarea) %>%
  summarize(count = sum(count, na.rm=TRUE)) %>% 
  group_by(date, surveyarea) %>% 
  mutate(prop = count / sum(count, na.rm=TRUE)) %>% 
  ...
  # filter(mat9points >= 4, mat9points <= 6, surveyarea == 3) %>% 
  # ggplot(aes(x=date, y=prop)) +
  # theme_publication() +
  # theme(strip.background = element_rect(colour=NA, fill = "#f0f0f0"),
  #       panel.border     = element_rect(colour="gray" , size=0.2),
  ...
  theme(strip.background = element_rect(colour=NA, fill = "#f0f0f0"),
        panel.border     = element_rect(colour="gray" , size=0.2),
        legend.title=element_blank() ) +
  geom_bar(aes(y=prop, fill=mat9points), stat="identity", position="fill") 
