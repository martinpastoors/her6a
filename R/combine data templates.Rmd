...
areas <- 
  read.csv(file=paste(dropboxdir,"/2018/DATA/Survey areas 2018.csv", sep=""), 
...
vesselnames <- data.frame(
    vessel     = c("KW172"    , "PH2200" , "SCH6",  "FR244"      , "PD365"      , "N905"   , "FR165"),
    vesselname = c("Dirk Dirk", "Wiron5+6", "Alida", "Christina S", "Kings Cross", "Voyager", "Unity"), 
...
file.list <- 
  list.files(path = paste(dropboxdir, "/2018/DATA/", sep=""),
...
# save RData file
save(haul, file=paste(dropboxdir, "/2018/DATA/rdata/haul.RData", sep=""))
...
bio <- 
  ...
  mutate(vessel = gsub("\\s+", "", str_trim(toupper(vessel))),
         ...
         mat    = as.numeric(mat),
         maturitystage19 = as.numeric(maturitystage19),
         
         # mat    = ifelse(maturitystage19 == 0    , NA, mat),
         mat    = ifelse(maturitystage19 %in% 2    , 1 , mat),
         mat    = ifelse(maturitystage19 %in% 3:4  , 2 , mat),
         mat    = ifelse(maturitystage19 %in% 5:6  , 3 , mat),
         mat    = ifelse(maturitystage19 %in% 7    , 4 , mat),
         mat    = ifelse(maturitystage19 %in% 8    , 5 , mat),
         mat    = ifelse(maturitystage19 %in% 9    , 6 , mat),
         age    = as.integer(age),
         wr     = as.integer(wr) ) %>% 
...
# save RData file
save(bio, file=paste(dropboxdir, "/2018/DATA/rdata/bio.RData", sep=""))
...
# save RData file
save(length, file=paste(dropboxdir, "/2018/DATA/rdata/length.RData", sep=""))
...
Spawning herring [ more text ] (Figure 3.2).

Length distributions of herring were largely similar by survey area and by haul (Figure 3.3, 3.4).

Focus on the time development in maturity of herring in all survey areas. All maturity data were converted into a common six point scale, in which stage 1 is immature, stage 2 is ripening, stage 3 is spawning and stage 4 is spent or resting.  (Figure 3.5). 
...
_Table 3.0: Total catch in 6a by area. _
...
t1 %>% 
  ...
  rename(vessel = vesselname2, begin = datestart, end=dateend, 
...
t1 %>% 
  left_join(t2, by=c("vesselname2","trip", "typehaul","haul","surveyarea")) %>% 
    rename(name = vesselname2, area=surveyarea, type=typehaul) %>% 
    pandoc.table(.,
             style = "simple",
             split.tables=400, 
             justify = "left",
...
haul %>%
  ...
  labs(x = NULL, y = NULL, title="2018 haul positions for each of the trips") +
  guides(size = guide_legend(nrow = 1)) +
  facet_wrap(~vesseltrip, ncol=3)
...
haul %>%
  ...
  labs(x = NULL, y = NULL, title="2018 haul positions of commercial (C) and survey (S) hauls") +
  guides(size = guide_legend(nrow = 1)) +
  facet_wrap(~typehaul)
...
_Figure 3.1: spatial distribution of commercial hauls (C) and survey hauls (S) in the 2018 herring survey_
...
t1 %>%
  ...
  labs(x = NULL, y = NULL, title="2018 haul positions with spawning herring, morphometrics and genetics") +
  guides(size = guide_legend(nrow = 1)) +
...
_Figure 3.2: spatial distribution of hauls with spawning herring, morphometric samples and genetic samples in the 2018 herring survey_
...
mat <- 
  ...
  filter(!is.na(mat)) %>% 
  filter(mat < 5) %>% 
  select(vessel, trip, haul, species, mat, age, sex) %>% 
...
_Figure 3.5: Proportion of maturity stage by date in all survey areas_
...
_Figure 3.6: wr-length keys by survey area_
